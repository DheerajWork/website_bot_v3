...
Scrapes Home/About/Contact pages, uses ChromaDB + OpenAI embeddings,
returns structured JSON for Business info
"""
...
async def fetch_page(url: str, headless: bool = USE_HEADLESS) -> str:
    """Load a webpage and return HTML"""
...
async def crawl_site(base_url: str, max_pages: int = 3) -> list:
    """Visit main pages only (Home/About/Contact)"""
...
openai_client = OpenAI(api_key=OPENAI_KEY) if OpenAI else None
...
def rag_extract(chunks, url):
    ...
    coll = chroma_client.get_or_create_collection(
        "rag_business_collection", embedding_function=openai_ef
    )
...
async def scrape_website(site_url: str) -> Dict:
    ...
    all_urls = await crawl_site(site_url, max_pages=3)
